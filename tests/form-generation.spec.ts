...
test.describe("Dynamic Form Generation", () => {
  const createTestSchema = (fields: any[] = []) =>
    JSON.stringify({
      formTitle: "Test Form",
  ...
  test.beforeEach(async ({ page }) => {
    await page.goto("https://animated-licorice-6f6e13.netlify.app/"); // Adjust to your app's URL
  });
  ...
  test("Generates textarea input", async ({ page }) => {
    ...
    const inputLabel = await page.locator(
      'label:has-text("Additional Comments")'
    );
    expect(await inputLabel.isVisible()).toBeTruthy();
  ...
  test("Generates multiple input types", async ({ page }) => {
    ...
    const textareaInput = await page.locator(`textarea[name=${JSON.parse(schema).fields[4].id}]`);
